...
awful.screen.connect_for_each_screen(function(s)
    ...
    for k,v in pairs(tags) do
      ...
      v.screen = s
      v.icon_only = true
     awful.tag.add(name, v)
    end

    -- Create a promptbox for each screen
    s.mypromptbox = awful.widget.prompt()
    -- Create an imagebox widget which will contain an icon indicating which layout we're using.
    -- We need one layoutbox per screen.animations
    ...
    layout   = {
        spacing = 1,
        spacing_widget = {
            color  = beautiful.scheme.foreground.normal,
    ...
    local top = awful.wibar({ position = "top", screen = s })
    local bottom = awful.wibar({ position = "bottom", screen = s })
    -- Create the wibox
    s.mywibox = gears.table.join(
      top,
      bottom
    )
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey }, "r", function ()
      awful.spawn("rofi -show run -theme /home/caellian/.config/awesome/theme/rofi/sidebar-dark.rasi")
